...
int test_dnssd_service_set_record()
{
	...
	rv = dnssd_service_add_txt_record_value(service, key, buf_len, buffer);
	if (rv != DNSSD_ERROR_NONE) {
		printf("Failed to set txt record value\n");
		return 0;
	}

	rv = dnssd_service_get_txt_record_value(service, &length, &data);
	if (rv != DNSSD_ERROR_NONE) {
	...
	rv = dnssd_service_remove_txt_record_value(service, key);
	if (rv != DNSSD_ERROR_NONE) {
		...
		dnssd_service_h remote_service, void *user_data)
{
	...
	if(service_state == DNSSD_SERVICE_STATE_AVAILABLE) {
		...
		printf("TXT Record    : ");
		dnssd_service_get_txt_record_value(remote_service, &txt_len,
				(void *)&txt_record);
