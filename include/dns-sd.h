...
/**
 * @brief Sets the DNS resource record. If the resource record for the type has
 *        been previously added using dnssd_service_set_record() then the
 *        record will be updated. DNS resource record can be set only after
 *        local_service is registered using dnssd_register_local_service()
 * @since_tizen 3.0
 * @privlevel public
 * @privilege http://tizen.org/privilege/nsd
 * @remarks You must pass only local services created using dnssd_create_local_service().
