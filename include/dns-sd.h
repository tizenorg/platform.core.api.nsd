...
/**
 * @brief Adds the TXT record. TXT record shoul be added after registering local
 *        service using dnssd_register_local_service().
 ...
 *          using dnssd_register_local_service()
 * @since_tizen 3.0
 * @privlevel public
 * @privilege http://tizen.org/privilege/nsd
 * @remarks You must pass only local services created using dnssd_create_local_service().
 ...
 * @param[in] data The data contained in resource record to be added, data is
 *            added using dnssd_service_add_txt_record_value() and then obtained
 *            using dnssd_service_get_txt_record_value() to pass here.
 * @return 0 on success, otherwise negative error value
 * @retval #DNSSD_ERROR_NONE Successful
 * @retval #DNSSD_ERROR_INVALID_PARAMETER Invalid parameter
